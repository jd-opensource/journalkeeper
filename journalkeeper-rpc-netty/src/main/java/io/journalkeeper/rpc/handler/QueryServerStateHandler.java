...
public class QueryServerStateHandler implements CommandHandler, Type {
    ...
    public Command handle(Transport transport, Command command) {
        try {
            serverRpc.queryServerState(GenericPayload.get(command.getPayload()))
                    ...
                    .thenAccept(response -> CommandSupport.sendResponse(response, RpcTypes.QUERY_SERVER_STATE_RESPONSE, command, transport));
        } catch (Throwable throwable) {
            return CommandSupport.newResponseCommand(new QueryStateResponse(throwable), RpcTypes.QUERY_SERVER_STATE_RESPONSE, command);
        }
        return null;
    }
    ...
    @Override
    public int type() {
        return RpcTypes.QUERY_SERVER_STATE_REQUEST;
    }
}
