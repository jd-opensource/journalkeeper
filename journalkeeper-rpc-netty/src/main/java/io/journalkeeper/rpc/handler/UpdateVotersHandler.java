 ...
 * Date: 2019-04-01
 */
public class UpdateVotersHandler implements CommandHandler, Type {
    private final ServerRpc serverRpc;

    public UpdateVotersHandler(ServerRpc serverRpc) {
    ...
    public Command handle(Transport transport, Command command) {
        try {
            serverRpc.updateVoters(GenericPayload.get(command.getPayload()))
                    .exceptionally(UpdateVotersResponse::new)
                    .thenAccept(response -> CommandSupport.sendResponse(response, RpcTypes.UPDATE_VOTERS_RESPONSE, command, transport));
        } catch (Throwable throwable) {
            return CommandSupport.newResponseCommand(new UpdateClusterStateResponse(throwable), RpcTypes.UPDATE_VOTERS_RESPONSE, command);
        }
        return null;
    }
