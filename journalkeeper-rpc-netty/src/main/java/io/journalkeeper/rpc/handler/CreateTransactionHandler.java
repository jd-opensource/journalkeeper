 ...
 * Date: 2019-04-01
 */
public class CreateTransactionHandler implements CommandHandler, Type {
    private final ServerRpc serverRpc;

    public CreateTransactionHandler(ServerRpc serverRpc) {
    ...
    public Command handle(Transport transport, Command command) {
        try {
            ...
            return CommandSupport.newResponseCommand(new CreateTransactionResponse(throwable), RpcTypes.CREATE_TRANSACTION_RESPONSE, command);
        }
        return null;
    }
    ...
    @Override
    public int type() {
        return RpcTypes.CREATE_TRANSACTION_REQUEST;
    }
}
