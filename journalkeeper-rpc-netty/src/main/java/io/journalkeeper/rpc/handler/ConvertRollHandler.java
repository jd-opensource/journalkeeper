...
public class ConvertRollHandler implements CommandHandler, Type {
    ...
    public Command handle(Transport transport, Command command) {
        try {
            serverRpc.convertRoll(GenericPayload.get(command.getPayload()))
                    .exceptionally(ConvertRollResponse::new)
                    .thenAccept(response -> CommandSupport.sendResponse(response, RpcTypes.CONVERT_ROLL_RESPONSE, command, transport));
        } catch (Throwable throwable) {
            return CommandSupport.newResponseCommand(new ConvertRollResponse(throwable), RpcTypes.CONVERT_ROLL_RESPONSE, command);
        }
        return null;
    }
