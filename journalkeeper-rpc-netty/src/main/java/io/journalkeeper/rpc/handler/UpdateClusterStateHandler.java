    ...
    public Command handle(Transport transport, Command command) {
        try {
            serverRpc.updateClusterState(GenericPayload.get(command.getPayload()))
                    ...
                    .thenAccept(response -> CommandSupport.sendResponse(response, RpcTypes.UPDATE_CLUSTER_STATE_RESPONSE, command, transport));
        } catch (Throwable throwable) {
            return CommandSupport.newResponseCommand(new UpdateClusterStateResponse(throwable), RpcTypes.UPDATE_CLUSTER_STATE_RESPONSE, command);
        }
        return null;
    }
