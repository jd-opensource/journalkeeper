 ...
 * Date: 2019-04-02
 */
public class InstallSnapshotHandler implements CommandHandler, Type {
    private final ServerRpc serverRpc;

    public InstallSnapshotHandler(ServerRpc serverRpc) {
    ...
    public Command handle(Transport transport, Command command) {
        try {
            serverRpc.installSnapshot(GenericPayload.get(command.getPayload()))
                    .exceptionally(InstallSnapshotResponse::new)
                    .thenAccept(response -> CommandSupport.sendResponse(response, RpcTypes.INSTALL_SNAPSHOT_RESPONSE, command, transport));
        } catch (Throwable throwable) {
            return CommandSupport.newResponseCommand(new InstallSnapshotResponse(throwable), RpcTypes.INSTALL_SNAPSHOT_RESPONSE, command);
        }
        return null;
    }
}
