 ...
 * Date: 2019-04-03
 */
public class MultiplePartitionJournalTest {
    private Path path = null;
    private MultiplePartitionJournal journal = null;
    ...
    public void writeReadEntryTest() {
        ...
        List<MultiplePartitionStorageEntry> storageEntries =
                entries.stream()
                        .map(entry -> new MultiplePartitionStorageEntry(entry, term, partition))
        ...
        while (index < journal.maxIndex()) {
            MultiplePartitionStorageEntry readStorageEntry = journal.readStorageEntry(index);
            MultiplePartitionStorageEntry writeStorageEntry = storageEntries.get((int) index);
    ...
    public void readByPartitionTest() throws IOException {
        ...
        Map<Short, List<MultiplePartitionStorageEntry>> partitionEntries =
        ...
        partitionEntries.forEach((partition, pEntries) -> {
            ...
            for (int i = 0; i < pEntries.size(); i++) {
                MultiplePartitionStorageEntry pEntry = pEntries.get(i);
    ...
    private byte[] serialize(MultiplePartitionStorageEntry storageEntry) {
        byte[] serialized = new byte[storageEntry.getLength()];
        MultiplePartitionStorageEntryParser.serialize(ByteBuffer.wrap(serialized), storageEntry);
        return serialized;
    }
    ...
    public void compareOrAppendTest() {
        ...
        List<MultiplePartitionStorageEntry> storageEntries =
                entries.stream()
                        .map(entry -> new MultiplePartitionStorageEntry(entry, 0))
                        .collect(Collectors.toList());
        ...
        List<MultiplePartitionStorageEntry> appendStorageEntries =
    ...
    private MultiplePartitionJournal createJournal() throws IOException {
       return createJournal(new Properties());
    }
    private MultiplePartitionJournal createJournal(Properties properties) throws IOException {
        ...
        MultiplePartitionJournal journal = new MultiplePartitionJournal(
                persistenceFactory,
                bufferPool);
